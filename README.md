...
> **一个人用 AI，从想法到上线。**  
> 不是「更多 prompt」，而是 **可验证的 Loop Engineering 流水线**——4 层循环架构、双层验证、最优停止策略，每个阶段有产物路径、门禁脚本与辩论 gate。
...
- Core: **18** native + **20** borrowed = **38** skills

**Platforms:** Cursor · Hermes · OpenCode · **Pipeline:** `pm-idea-to-mvp` v9.2.0

**文档索引：** [ARCHITECTURE](docs/ARCHITECTURE.md) · [CODING_CONVENTIONS](docs/CODING_CONVENTIONS.md) · [DEPLOY_CONVENTIONS](docs/DEPLOY_CONVENTIONS.md) · [HERMES_ARCHITECTURE](docs/HERMES_ARCHITECTURE.md) · [GIT_WORKFLOW](docs/GIT_WORKFLOW.md)
...
v9.2 从「双循环方法论」升级为 **4 层循环架构**——方法论不变，执行层有了明确的分层：

| 层 | 名称 | 职责 | 核心洞察 |
|---|------|------|---------|
| **Loop 1** | Agent Loop | 模型重复调用工具直到任务完成 | 自动化执行 |
| **Loop 2** | Verification Loop | 双层验证（LLM 改进 + 确定性硬停止） | **验证器才是产品，其他都是管道** |
| **Loop 3** | Event-Driven Loop | 事件触发代理运行（webhook、cron、新数据） | 规模化集成 |
| **Loop 4** | Hill Climbing Loop | 分析生产 traces，自动重写 harness 配置 | 自进化闭环 |

> 核心洞察：**输出质量受限于验证器质量，不会更高。** 所以 Loop 2 是核心产品，其余都是管道。
...
- **G1/G2 辩论** → grill/红队技能 + 产物路径，**不占用人工 unblock**
...
**问题**：固定 3 次内循环迭代，缺少质量-效率权衡。  
**方案**：动态迭代 + 质量收敛判断——

- Iteration 1: 基础功能通过 → 继续
- Iteration 2: 边界情况覆盖 → 继续
- Iteration 3: 性能优化 → 评估收敛
- Iteration 4+: 仍有问题 → 记录 `feedback.jsonl`，人工介入

### 5. 少而精的技能层

18 native + 20 borrowed = **38** core skills：
...
- **deploy**：[DEPLOY_CONVENTIONS.md](docs/DEPLOY_CONVENTIONS.md)
- **通知**：`feishu_notify.py`

### 8. Agent 行为准则与实战强化（v7.0 / v7.1 / v7.2）
...
| **Stage → Skill 映射** | SKILL.md 内 Pipeline stages 表格（v9 无 stage-skills.yaml） |
...
3. 读 `pipelines/pm-idea-to-mvp/SKILL.md` + 当前 stage 卡片
...
| 单阶段 | 进入 spec / mvp 阶段 | 只加载该 stage 技能（见 SKILL.md） |

### 阶段边界（v9.2）

验证 SKILL.md 产物路径存在 → 更新 PROGRESS.md。
...
| ------ | -------- | ---------------------------------------------------------------- | ----------- |
| **G1** | align    | `debates/align-synthesis.md exists (G1)`（含 `debate_resolved`）               | 假设被挑战过，不是自嗨 |
| **G2** | spec     | `debates/spec-synthesis.md exists (G2)`（含 PRD 辩论）                           | PRD 经红队审查   |
...
```mermaid
flowchart TB
  subgraph L0 [L0_Core_Scripts]
    IL[inner-loop-driver.py]
    IP[init-project.py]
    CF[consume-feedback.py]
  end
  subgraph L1 [L1_Pipeline]
    PM[pm-idea-to-mvp SKILL.md]
  end
  subgraph L2 [L2_Native_18]
    Skills[grill / c4 / openspec / remote-server-deployment ...]
  end
  subgraph L3 [L3_Borrowed_20]
    Phuryn[pm-*]
    KW[kw-*]
  end
  subgraph L4 [L4_DevOps]
    DevOps[cloudflare-tunnel / kanban-admin / task-state-persist ...]
  end
  PM --> Skills
  PM --> L0
  L3 --> Skills
...
### Native 18（ttmens 差异化）

| 技能 | Stage | 能力 |
|------|-------|------|
| **pm-idea-to-mvp** | all | 唯一主流水线 v9.2，4 层循环 + 最优停止 |
| **remote-server-deployment** | ship | SSH 部署（registry 优先） |
...
### DevOps 运维技能（Hermes 基础设施）

不在 pipeline 阶段链中，但支撑日常运维：

| 技能 | 能力 |
|------|------|
| **cloudflare-tunnel** | 临时/持久 Cloudflare Tunnel 管理 |
| **deploy-verify** | 部署后自动化验证（健康检查、端口、SSL） |
| **flutter-android-build** | Flutter Android APK 远程构建 |
| **kanban-admin** | Hermes Kanban 批量运维与清理 |
| **task-state-persist** | 关键任务状态持久化（防上下文压缩丢失） |
| **infra-ops** | 基础设施运维知识 SSOT（deploy-servers、SSH 认证等） |
...
| `init-project.py`        | 初始化项目目录结构（CONTEXT.md, decisions.md 等）  |
| `consume-feedback.py`    | retro 消费 feedback.jsonl 闭环                 |
| `harness-rules.yaml`     | 决策规则配置（风险分级、自动化策略）                |
...
| `scripts/feishu_notify.py`         | 阶段完成通知（Open API + webhook fallback） |
...
- **Feishu** — `feishu_notify.py` 推送摘要  
- **Hermes Kanban** — 通用任务分发（非 PM 编排脚本）
...
跨平台说明：[docs/platforms/cursor.md](docs/platforms/cursor.md) · [hermes.md](docs/platforms/hermes.md)
...
python scripts/validate_skills.py       # marketplace + layout + v9.2 脚本
