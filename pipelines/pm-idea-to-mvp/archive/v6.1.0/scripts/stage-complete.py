...
def main():
    ...
    parser.add_argument(
        "--run", default=None,
        help="Alias for --project-root (v5.x SOUL compatibility)"
    )
    parser.add_argument(
        "--slug", default="",
        help="Optional slug hint (derived from project dir name if omitted)"
    )
    parser.add_argument(
        "--task-id", default="",
    ...
    project_root_arg = args.project_root or args.run
    if not project_root_arg:
        print(json.dumps({"error": "Provide --project-root or --run"}))
        sys.exit(2)
